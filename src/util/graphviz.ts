...
import { getDependencies, getDependenciesOf } from '../builder/query';


import { Site } from '../builder/site';
import { ProcessOptions } from '../builder/types';
import { Entity, EntityId, isEntityId, Component, ComponentDef, componentToObject } from '../es';


const Label = '/util/graph';
...
export async function process(site: Site, options: GraphOptions = {}) {
    const { es } = site;
    ...
    if (options.dstOnly) {
        ...
        for (const [key, [eid]] of idx) {
            ...
            for (const dep of deps) {
                ...
                // let { node, deps: sdeps } = entityToNode(g, es, dep, { ...options, srcOnly: true });
...
async function addEntity(g: Digraph, es: EntitySet, id: EntityId|Entity, nodes, edges, options: GraphOptions) {
...
function entityToNode(g: Digraph, es: EntitySet, e: Entity, options: GraphOptions) {
